...
String fromdt="",todt="",opdtype="A",doctorobjid="";
...
if(request.getParameter("Submit")!=null){
	...
	if(opdtype.equals("A")){
		strQry_FS = strQry + " and stafffree in('S','F','B')" ;
	...
	String strdate = appUtility.GetSystemDateToDisplay();
	strQry="select * from mst_opd_registration where createddate between '" + strdate + "' and '" + strdate + "' and stafffree not in('S','F')" ;
	strQry_FS="select * from mst_opd_registration where createddate between '" + strdate + "' and '" + strdate + "' and stafffree in('S','F')" ;
}	
...
<body topmargin="10px">
<form name="frmOPDRegList" method="post" action="OpdRegistrationList.jsp">
						...
						<td class="Font8vB" align="center" style="HEIGHT:30px"> 
							<table width="100%" border="0" cellspacing="1" cellpadding="1" style="border: 1px solid #6699FF;">
							  ...
							  <tr>
								...
								<td>
									<input class="mandtxtbox" type="text" name="fromDT" id="fromDT" maxlength="12" size="12" value="<%if(fromdt!=null && fromdt.length() == 0){%><%=appUtility.ChangeDateFormatToMySQL(fromdt)%><%}else{%><%=fromdt%><%}%>" onFocus="blur();"/>
								...
								<td>
									<input class="mandtxtbox" type="text" name="toDT" id="toDT" maxlength="12" size="12" value="<%if(todt!=null && todt.length() == 0){%><%=appUtility.ChangeDateFormatToMySQL(todt)%><%}else{%><%=todt%><%}%>" onFocus="blur();"/>
							...
							<table class="input" border="0" cellpadding="2"width="100%">
							  <tr class="lblhdr"> 
								...
								<td align="center">
								  ...
								  int mtot=0;
								  System.out.println("opdtype===" + opdtype + "=====strQry===" + strQry);	
								  try{								  
									opdregistration.Connect();
