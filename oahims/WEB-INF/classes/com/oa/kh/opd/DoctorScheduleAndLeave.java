...
public class DoctorScheduleAndLeave {
    Connection con = null;
    String error;
    String msg;

    public DoctorScheduleAndLeave() {
    }
    ...
    public int checkDoctorScheduleAndLeaveId(String doctorobjid,String chkOnDate){
        int no=0;
        if(con != null)
        {
             try
             {
                 ...
                 ResultSet rs = null;
                 rs=st.executeQuery("select count(*) as mCnt FROM doctor_schedule where doctorobjid="+doctorobjid + " and dscheduleorleavedate ='" + chkOnDate + "'");
                 while(rs.next()){
                     no = rs.getInt("mCnt");
                     System.out.println("checkDoctorScheduleAndLeaveId ===" + no);
                 }
                 rs.close();
             }catch(Exception e)
             {
	...
	public ResultSet listDoctorScheduleAndLeaveId(String doctorobjid){
		ResultSet rs = null;
		if(con != null)
		{
			 try
			 {
				 Statement st=con.createStatement();
				 if(doctorobjid != null ){
				 rs=st.executeQuery("select * FROM doctor_schedule where doctorobjid="+doctorobjid);
			 	 }else{
				 rs=st.executeQuery("select * FROM doctor_schedule");
			 	 }
